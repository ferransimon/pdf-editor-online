import type { Translations } from "./en";

export const es: Translations = {
  modeSelector: {
    subtitle: "¿Qué quieres hacer con tus PDFs?",
    split: "Dividir",
    splitDesc: "Separa un PDF en múltiples archivos independientes",
    merge: "Unir",
    mergeDesc: "Combina varios PDFs en uno y reordena sus páginas",
    annotate: "Anotar",
    annotateDesc: "Añade texto, dibujos y formas sobre las páginas",
    compress: "Comprimir",
    compressDesc: "Reduce el tamaño del archivo PDF con optimización sin pérdida",
    privacy: "100% privado.",
    privacyDesc:
      "Todos los archivos se procesan directamente en tu navegador. Ningún PDF es enviado ni almacenado en ningún servidor.",
  },
  uploader: {
    errorNotPdf: "Por favor, selecciona un archivo PDF.",
    errorOpen: "Error al abrir el archivo.",
    loading: "Cargando PDF…",
    back: "Volver",
    splitTitle: "Dividir PDF",
    splitSubtitle:
      "Divide tu PDF en partes y descárgalas como ZIP. Todo ocurre en tu navegador.",
    annotateTitle: "Anotar PDF",
    annotateSubtitle:
      "Añade texto, dibujos y formas sobre las páginas de tu PDF. Todo ocurre en tu navegador.",
    compressTitle: "Comprimir PDF",
    compressSubtitle:
      "Reduce el tamaño de tu archivo PDF con optimización sin pérdida. Todo ocurre en tu navegador.",
    dragHere: "Arrastra un PDF aquí",
    dragOr: "o haz clic para seleccionar",
    select: "Seleccionar PDF",
  },
  toolbar: {
    sectionSingular: "sección",
    sectionPlural: "secciones",
    new: "Nuevo",
    generating: "Generando…",
    downloadZip: "Descargar ZIP",
  },
  sectionList: {
    sections: "Secciones",
    section: "Sección",
    pageSingular: "Pág.",
    pagePlural: "Págs.",
  },
  sectionPreview: {
    selectSection: "Selecciona una sección",
    pageSingular: "Página",
    pagePlural: "Páginas",
  },
  merge: {
    back: "Volver",
    title: "Unir PDFs",
    subtitle: "Selecciona 2 o más PDFs. Luego podrás reordenar las páginas.",
    dragHere: "Arrastra los PDFs aquí",
    dragOr: "o haz clic para seleccionarlos",
    selectPdfs: "Seleccionar PDFs",
    errorSelect: "Selecciona al menos un archivo PDF.",
    pageSingular: "página",
    pagePlural: "páginas",
    fileSingular: "archivo",
    filePlural: "archivos",
    filenamePlaceholder: "nombre del archivo",
    addPdfs: "Añadir PDFs",
    generating: "Generando…",
    downloadPdf: "Descargar PDF",
    allPagesDeleted: "Todas las páginas han sido eliminadas.",
    pageShort: "Pág.",
    loadingPreview: "Cargando…",
  },
  annotate: {
    back: "Volver",
    draw: "Dibujar",
    text: "Texto",
    rect: "Rectángulo",
    insertImage: "Insertar imagen",
    image: "Imagen",
    fontSize: "Tamaño",
    lineWidthTitle: (w: number) => `Grosor ${w}px`,
    undoTitle: "Deshacer última anotación",
    undo: "Deshacer",
    print: "Imprimir",
    loadingPdf: "Cargando PDF…",
    pageOf: (cur: number, total: number) => `Página ${cur} de ${total}`,
    confirm: "Confirmar",
    cancel: "Cancelar",
    popupsAlert: "Activa las ventanas emergentes para poder imprimir.",
    printDocTitle: "imprimir",    previous: "Anterior",
    next: "Siguiente",
    addTextTitle: "A\u00f1adir texto",
    textPlaceholder: "Escribe el texto aqu\u00ed\u2026",
    textHint: "El texto aparecer\u00e1 en el centro de la p\u00e1gina. Podr\u00e1s arrastrarlo al lugar exacto.",
    insertText: "Insertar texto",
    currentColor: "Color actual",
    cloneButton: "Clonar en páginas",
    cloneModal: {
      title: "Clonar anotaciones en páginas",
      selectAll: "Seleccionar todo",
      deselectAll: "Deseleccionar todo",
      apply: "Aplicar",
      cancel: "Cancelar",
      currentPage: "actual",
      selected: (n: number) => `${n} página${n === 1 ? "" : "s"} seleccionada${n === 1 ? "" : "s"}`,
    },
  },
  compress: {
    back: "Volver",
    title: "Comprimir PDF",
    currentSize: "Tamaño actual",
    compressionLevel: "Nivel de compresión",
    level_low: "Bajo",
    level_low_desc: "Compresión mínima. Procesamiento más rápido.",
    level_medium: "Medio (Recomendado)",
    level_medium_desc: "Compresión equilibrada con optimización de flujos de objetos.",
    level_high: "Alto",
    level_high_desc: "Compresión máxima. Elimina metadatos.",
    level_extreme: "Extremo (Con pérdida)",
    level_extreme_desc: "Compresión agresiva. Convierte páginas a imágenes. Pierde texto seleccionable.",
    infoMessage: "La compresión es sin pérdida. La reducción real depende de qué tan optimizado esté ya tu PDF.",
    extremeWarningTitle: "⚠️ Advertencia: Compresión con pérdida",
    extremeWarning: "Este modo convierte cada página a una imagen JPEG. Perderás texto seleccionable, gráficos vectoriales, enlaces y otras características del PDF. Ideal para documentos escaneados o PDFs con muchas imágenes.",
    compressButton: "Comprimir PDF",
    compressing: "Comprimiendo PDF…",
    compressionComplete: "¡Compresión Completada!",
    originalSize: "Tamaño original",
    compressedSize: "Tamaño comprimido",
    saved: "Has ahorrado",
    increased: "Tamaño aumentado",
    download: "Descargar",
    downloading: "Descargando…",
    tryAgain: "Intentar de Nuevo",
    warningLarger: "El archivo comprimido es ligeramente más grande. Esto puede ocurrir con PDFs ya optimizados. Aún puedes descargarlo.",
    errorCompressing: "Error al comprimir el PDF. Por favor, inténtalo de nuevo.",
  },
  sidebar: {
    pages: (n: number) => `P\u00e1ginas (${n})`,
    removeSplit: "Quitar divisi\u00f3n",
    splitHere: "Dividir aqu\u00ed",
    pageAlt: (n: number) => `P\u00e1gina ${n}`,  },
  mobile: {
    title: "Diseñado para escritorio",
    desc: "Esta aplicación no está optimizada para dispositivos móviles. Para una experiencia completa, ábrela desde un ordenador.",
    continue: "Continuar de todas formas",
  },
  donate: {
    label: "Invítame a un café ☕",
  },
  theme: {
    toLight: "Cambiar a modo claro",
    toDark: "Cambiar a modo oscuro",
  },
  privacy: {
    link: "Pol\u00edtica de privacidad",
    title: "Pol\u00edtica de Privacidad",
    lastUpdated: "\u00daltima actualizaci\u00f3n: 5 de marzo de 2026",
    intro:
      "Esta pol\u00edtica de privacidad explica qu\u00e9 datos se recopilan cuando usas PDF Editor Online y c\u00f3mo se utilizan.",
    section1Title: "1. Procesamiento local de archivos",
    section1Body:
      "Todas las operaciones con PDFs (dividir, unir, anotar) se realizan \u00edntegramente en tu navegador. Tus archivos nunca se suben a ning\u00fan servidor. No tenemos acceso a los documentos que procesas.",
    section2Title: "2. Anal\u00edtica (Vercel Analytics)",
    section2Body:
      "Utilizamos Vercel Analytics para entender c\u00f3mo se usa la aplicaci\u00f3n. Este servicio funciona en modo sin cookies por defecto: no instala ninguna cookie ni identificador de rastreo en tu dispositivo. Los datos recopilados son agregados y an\u00f3nimos e incluyen:",
    section2List: [
      "P\u00e1gina visitada",
      "Pa\u00eds derivado de la IP (la direcci\u00f3n IP no se almacena)",
      "Tipo de navegador y sistema operativo",
      "URL de referencia",
    ],
    section2Footer:
      "Vercel no almacena ninguna direcci\u00f3n IP. Puedes consultar la pol\u00edtica de privacidad de Vercel en",
    section3Title: "3. M\u00e9tricas de rendimiento (Vercel Speed Insights)",
    section3Body:
      "Usamos Vercel Speed Insights para monitorizar el rendimiento de la p\u00e1gina (Core Web Vitals). Al igual que Analytics, funciona sin cookies y no almacena informaci\u00f3n de identificaci\u00f3n personal.",
    section4Title: "4. Servicios de terceros",
    section4Body:
      "El \u00fanico servicio de terceros que recibe datos es Vercel Inc. (con sede en San Francisco, EE.UU.), que act\u00faa como proveedor de alojamiento y anal\u00edtica. Vercel cumple con el RGPD.",
    section5Title: "5. Tus derechos (RGPD)",
    section5Body:
      "Dado que no recopilamos informaci\u00f3n de identificaci\u00f3n personal y no se utilizan cookies, no existe ning\u00fan perfil de datos personales asociado a ti. Si tienes alguna pregunta, puedes contactarnos a trav\u00e9s del repositorio p\u00fablico:",
    repoLabel: "Ver el proyecto en GitHub",
    section6Title: "6. Cambios en esta pol\u00edtica",
    section6Body:
      "Podemos actualizar esta pol\u00edtica a medida que evolucione la aplicaci\u00f3n. La fecha al inicio de esta p\u00e1gina indica cu\u00e1ndo fue revisada por \u00faltima vez.",
    backHome: "\u2190 Volver al editor",
  },
};
