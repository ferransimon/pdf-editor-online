export const en = {
  modeSelector: {
    subtitle: "What do you want to do with your PDFs?",
    split: "Split",
    splitDesc: "Separate a PDF into multiple independent files",
    merge: "Merge",
    mergeDesc: "Combine several PDFs into one and reorder pages",
    annotate: "Annotate",
    annotateDesc: "Add text, drawings and shapes to pages",
    compress: "Compress",
    compressDesc: "Reduce PDF file size with lossless optimization",
    privacy: "100% private.",
    privacyDesc:
      "All files are processed directly in your browser. No PDF is sent or stored on any server.",
  },
  uploader: {
    errorNotPdf: "Please select a PDF file.",
    errorOpen: "Error opening the file.",
    loading: "Loading PDF…",
    back: "Back",
    splitTitle: "Split PDF",
    splitSubtitle:
      "Split your PDF into parts and download them as a ZIP. Everything happens in your browser.",
    annotateTitle: "Annotate PDF",
    annotateSubtitle:
      "Add text, drawings and shapes to your PDF pages. Everything happens in your browser.",
    compressTitle: "Compress PDF",
    compressSubtitle:
      "Reduce your PDF file size with lossless optimization. Everything happens in your browser.",
    dragHere: "Drag a PDF here",
    dragOr: "or click to select",
    select: "Select PDF",
  },
  toolbar: {
    sectionSingular: "section",
    sectionPlural: "sections",
    new: "New",
    generating: "Generating…",
    downloadZip: "Download ZIP",
  },
  sectionList: {
    sections: "Sections",
    section: "Section",
    pageSingular: "Pg.",
    pagePlural: "Pgs.",
  },
  sectionPreview: {
    selectSection: "Select a section",
    pageSingular: "Page",
    pagePlural: "Pages",
  },
  merge: {
    back: "Back",
    title: "Merge PDFs",
    subtitle: "Select 2 or more PDFs. Then you can reorder the pages.",
    dragHere: "Drag PDFs here",
    dragOr: "or click to select them",
    selectPdfs: "Select PDFs",
    errorSelect: "Please select at least one PDF file.",
    pageSingular: "page",
    pagePlural: "pages",
    fileSingular: "file",
    filePlural: "files",
    filenamePlaceholder: "file name",
    addPdfs: "Add PDFs",
    generating: "Generating…",
    downloadPdf: "Download PDF",
    allPagesDeleted: "All pages have been removed.",
    pageShort: "Pg.",
    loadingPreview: "Loading…",
  },
  annotate: {
    back: "Back",
    draw: "Draw",
    text: "Text",
    rect: "Rectangle",
    insertImage: "Insert image",
    image: "Image",
    fontSize: "Size",
    lineWidthTitle: (w: number) => `Width ${w}px`,
    undoTitle: "Undo last annotation",
    undo: "Undo",
    print: "Print",
    loadingPdf: "Loading PDF…",
    pageOf: (cur: number, total: number) => `Page ${cur} of ${total}`,
    confirm: "Confirm",
    cancel: "Cancel",
    popupsAlert: "Enable pop-ups to print.",
    printDocTitle: "print",    previous: "Previous",
    next: "Next",
    addTextTitle: "Add text",
    textPlaceholder: "Write your text here\u2026",
    textHint: "The text will appear at the center of the page. You can drag it to the exact position.",
    insertText: "Insert text",
    currentColor: "Current color",
    cloneButton: "Clone to pages",
    cloneModal: {
      title: "Clone annotations to pages",
      selectAll: "Select all",
      deselectAll: "Deselect all",
      apply: "Apply",
      cancel: "Cancel",
      currentPage: "current",
      selected: (n: number) => `${n} page${n === 1 ? "" : "s"} selected`,
    },
  },
  compress: {
    back: "Back",
    title: "Compress PDF",
    currentSize: "Current size",
    compressionLevel: "Compression level",
    level_low: "Low",
    level_low_desc: "Minimal compression. Fastest processing.",
    level_medium: "Medium (Recommended)",
    level_medium_desc: "Balanced compression with object stream optimization.",
    level_high: "High",
    level_high_desc: "Maximum compression. Removes metadata.",
    level_extreme: "Extreme (Lossy)",
    level_extreme_desc: "Aggressive compression. Converts pages to images. Loses text selectability.",
    infoMessage: "Compression is lossless. The actual reduction depends on how optimized your PDF already is.",
    extremeWarningTitle: "⚠️ Warning: Lossy compression",
    extremeWarning: "This mode converts each page to a JPEG image. You will lose text selectability, vector graphics, links, and other PDF features. Best for scanned documents or image-heavy PDFs.",
    compressButton: "Compress PDF",
    compressing: "Compressing PDF…",
    compressionComplete: "Compression Complete!",
    originalSize: "Original size",
    compressedSize: "Compressed size",
    saved: "You saved",
    increased: "Size increased",
    download: "Download",
    downloading: "Downloading…",
    tryAgain: "Try Again",
    warningLarger: "The compressed file is slightly larger. This can happen with already-optimized PDFs. You can still download it.",
    errorCompressing: "Error compressing the PDF. Please try again.",
  },
  sidebar: {
    pages: (n: number) => `Pages (${n})`,
    removeSplit: "Remove split",
    splitHere: "Split here",
    pageAlt: (n: number) => `Page ${n}`,  },
  mobile: {
    title: "Designed for desktop",
    desc: "This app is not optimized for mobile devices. For the full experience, open it from a computer.",
    continue: "Continue anyway",
  },
  donate: {
    label: "Buy me a coffee ☕",
  },
  theme: {
    toLight: "Switch to light mode",
    toDark: "Switch to dark mode",
  },
  privacy: {
    link: "Privacy policy",
    title: "Privacy Policy",
    lastUpdated: "Last updated: March 5, 2026",
    intro:
      "This privacy policy explains what data is collected when you use PDF Editor Online and how it is used.",
    section1Title: "1. Local file processing",
    section1Body:
      "All PDF operations (split, merge, annotate) are performed entirely in your browser. Your files are never uploaded to any server. We have no access to the documents you process.",
    section2Title: "2. Analytics (Vercel Analytics)",
    section2Body:
      "We use Vercel Analytics to understand how the application is used. This service works in cookieless mode by default: it does not set any cookies or tracking identifiers on your device. The data collected is aggregated and anonymous and includes:",
    section2List: [
      "Page visited",
      "Country derived from IP (IP address is not stored)",
      "Browser and operating system type",
      "Referrer URL",
    ],
    section2Footer:
      "No IP address is stored by Vercel. You can read Vercel\u2019s privacy policy at",
    section3Title: "3. Performance metrics (Vercel Speed Insights)",
    section3Body:
      "We use Vercel Speed Insights to monitor page performance (Core Web Vitals). Like Analytics, it works without cookies and does not store personally identifiable information.",
    section4Title: "4. Third-party services",
    section4Body:
      "The only third-party service that receives data is Vercel Inc. (based in San Francisco, USA), acting as our hosting and analytics provider. Vercel is compliant with GDPR.",
    section5Title: "5. Your rights (GDPR)",
    section5Body:
      "Since we do not collect personally identifiable information and no cookies are used, there is no personal data profile associated with you. If you have any questions, you can reach us through the public repository:",
    repoLabel: "View the project on GitHub",
    section6Title: "6. Changes to this policy",
    section6Body:
      "We may update this policy as the application evolves. The date at the top of this page indicates when it was last revised.",
    backHome: "\u2190 Back to the editor",
  },
};

export type Translations = typeof en;
